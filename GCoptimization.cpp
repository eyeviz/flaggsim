...
bool GCoptimization::solveSpecialCases(EnergyType& energy)
{
    ...
    if ( !dc && !sc && !lc ) {
	printf( "&&&&&&&&&& No Data costs; No connections; No labels\n" );
	energy = 0;
	return true;
    }

    if ( dc && !sc && !lc ) {
	printf( "&&&&&&&&&& No connections; No labels\n" );
...
GCoptimization::EnergyType GCoptimization::solveGreedy()
{
...
#ifdef SKIP
	printf( " Data cost : " );
	for ( DataCostT * ptrDC = ( DataCostT * )m_datacostFn; ptrDC; ptrDC = ptrDC->next)
	    printf( "DC = %f ", *ptrDC );
	printf( "\n" );
#endif	// SKIP
	...
	// This has been modified for label cost optimization for
	// aggregating buidling polygons
	// noted by ST on 2023/11/11
	for ( SiteID i = 0; i < m_num_sites; ++i ) {
	    m_labeling[i] = i;
	    m_labelingDataCosts[i] = dc->compute(i,i);
	    for ( LabelCostIter* lci = m_labelcostsByLabel[i]; lci; lci = lci->next )
		lci->node->active = true;
	}
...
void GCoptimization::printStatus2(int alpha, int beta, int numVars, gcoclock_t ticks0)
{
    ...
    // My debug
    for ( unsigned int k = 0; k < m_num_sites; ++k )
	printf( "%3d ", m_labeling[ k ] );
    printf( "\n" );

#ifdef ENABLE_TRACKING
    int totalCost = compute_energy();
    if ( ( nTracks == 0 ) ||
	 ( totalCost < trackCost[ nTracks - 1 ][ 0 ] ) ) {
	assert( m_num_sites <= MAX_PIXELS ); 
	for ( unsigned int k = 0; k < m_num_sites; ++k ) {
	    trackLabel[ nTracks ][ k ] = m_labeling[ k ];
	}
	trackCost[ nTracks ][ 0 ] = totalCost;
	trackCost[ nTracks ][ 1 ] = giveDataEnergy();
	trackCost[ nTracks ][ 2 ] = giveSmoothEnergy();
	trackCost[ nTracks ][ 3 ] = giveLabelEnergy();
	nTracks++;
    }
#endif	// ENABLEL_TRACKING
